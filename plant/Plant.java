...
public class Plant extends Sprite {
     ...
     * 第一次购买植物是否无购买冷却时间
     */
    public boolean firstPurchaseNoCooldown = true;
    /**
    ...
    public void updatePlantChosen(long time, GraphicsContext context) {
        ...
        double cooldownPercentage;
        if(firstPurchaseNoCooldown && Game.sun >= sunCost){
            cooldownPercentage = 100;
            firstPurchaseNoCooldown = false;
        }
        else if(Game.sun >= sunCost && !firstPurchaseNoCooldown) {
            cooldownPercentage = (elapsedTime / (cooldownTime * 10)) > 100 ? 100 : (elapsedTime / (cooldownTime * 10));
        }else{
