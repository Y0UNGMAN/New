...
public class Zombie extends Sprite {
    ...
    /**
     * 是否是第一个登场的僵尸，用来播放开始战斗的音乐
     */
    static boolean isFirstZombie = true;
    ...
    public static void generateZombie() {
        ...
        if (Game.currentZombie < Stage.stageZombie.length && Game.gameElapsedtime >= Stage.stageZombie[Game.currentZombie][0] * 1000L) {
            ...
            if(isFirstZombie){
                SoundManager.playSound("/resources/music/ZombieComing.mp3");
                isFirstZombie = false;
            }
     ...
     * 更新并渲染当前僵尸的血条。
     *
     * <p>根据僵尸的类型和当前生命值，计算血条的宽度，并根据血量的不同阶段（高、中、低）渲染不同的颜色。</p>
     *
     * @param context 用于渲染的<code>GraphicsContext</code>对象。
     * @param zombieType 当前僵尸的类型，用于判断该僵尸的最大生命值，以便正确计算血条宽度。
     */
