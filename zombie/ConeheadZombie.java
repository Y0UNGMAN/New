...
/**
 * <p>
 *     <code>ConeheadZombie</code>类为僵尸<code>ConeheadZombie</code>的具体实现类。
 * </p>
 */
public class ConeheadZombie extends Zombie {
    /**
     * <p>
     *     创建<code>ConeheadZombie</code>对象。
     * </p>
     * <p>
     *     创建<code>ConeheadZombie</code>对象时，会对<code>ConeheadZombie</code>的一些成员变量对应地进行初始化。
     *     <code>ConeheadZombie</code>会被随机地创建在第1至第5行草地的其中一行上。
     * </p>
     * @see Zombie
     */
    public ConeheadZombie() {
        super();
        health = Constant.CONEHEAD_ZOMBIE_HEALTH;
        damage = Constant.CONEHEAD_ZOMBIE_DAMAGE;
        attackSpeed = Constant.CONEHEAD_ZOMBIE_ATTACK_SPEED;
        setImage(Constant.CONEHEAD_ZOMBIE_MOVING_IMAGE_1);
        setVelocity(Constant.CONEHEAD_ZOMBIE_MOVEMENT_SPEED, 0);
    ...
    /**
     * <p>
     *     更新<code>ConeheadZombie</code>对象的属性及状态，也对<code>ConeheadZombie</code>的攻击进行更新与判定。
     * </p>
     * <p>
     *     此方法也把<code>ConeheadZombie</code>对象渲染到屏幕上。
     *     要是<code>ConeheadZombie</code>与植物产生碰撞时，就会停止移动，开始攻击植物，直到植物血量归0或是<code>ConeheadZombie</code>死亡为止。
     *     要是<code>ConeheadZombie</code>被减速了，则其移速和攻速会变为原本的值的一半。
     * </p>
     * @param time 时间
    ...
    public void update(long time, GraphicsContext context) {
        ...
        if (isSlowDown) {
            setVelocity(Constant.CONEHEAD_ZOMBIE_MOVEMENT_SPEED / 2, 0);
            attackSpeed = Constant.CONEHEAD_ZOMBIE_ATTACK_SPEED / 2;
        } else {
            setVelocity(Constant.CONEHEAD_ZOMBIE_MOVEMENT_SPEED, 0);
            attackSpeed = Constant.CONEHEAD_ZOMBIE_ATTACK_SPEED;
        }
        ...
        for (Plant plant : Game.plantList) {
            if (this.isOverlapped(plant) && !(plant instanceof CherryBomb)) {
                ...
                changePic = (int)(time%(Constant.CONEHEAD_ZOMBIE_ATTACK_IMAGE.length-1));
                setImage(Constant.CONEHEAD_ZOMBIE_ATTACK_IMAGE[changePic]);
        ...
        if (flag == 0) {
            changePic = (int)(time%(Constant.CONEHEAD_ZOMBIE_MOVING_IMAGE.length-1));
            setImage(Constant.CONEHEAD_ZOMBIE_MOVING_IMAGE[changePic]);
        ...
        this.updateHealthBar(context,Constant.CONEHEAD_ZOMBIE);
        this.render(context);
    }
}
